...
async def ip(ctx, *arg):
    if len(arg) == 2:
        await ctx.send(calc_ip(arg[0], arg[1]))
    elif len(arg) == 1:
        await ctx.send('```Допиши <номер маски>, а то я хз, что с этим делать)```')
    elif len(arg) == 0:
        await ctx.send('```Гайд как ввести команду ip: <ip-адрес> <номер маски>```')
    elif len(arg) > 2:
        await ctx.send('```В глаза долбишься? Написано же - введи <ip-адрес> <номер маски>```')
